...
class TestTrackerAntiJump:
    ...
    def test_far_false_detection_does_not_hijack_track(self):
        ...
        centers = [((o[1][0] + o[1][2]) / 2, (o[1][1] + o[1][3]) / 2) for o in out]
        # ни один наблюдаемый центр не должен оказаться у ложной позиции
        assert all(math.hypot(cx - 110, cy - 110) < 60 for cx, cy in centers), \
            "трекер не должен мгновенно перескакивать на далёкую ложную детекцию"
